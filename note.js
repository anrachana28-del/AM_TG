...
import { downloadProfilePhoto } from "telegram/helpers.js"; // use Telegram helper to get photo
    ...
    try {
      ...
      for await (const user of client.iterParticipants(groupEntity)) {
        try {
          ...
          if (user.photo) {
            try {
              const buffer = await downloadProfilePhoto(client, user, { isBig: false });
              if (buffer && buffer.length > 0) {
                profilePhotoURL = `data:image/jpeg;base64,${Buffer.from(buffer).toString('base64')}`;
              }
            } catch (e) {
              console.log(`⚠️ Failed to download photo for ${user.username || user.id}: ${e.message}`);
            }
          }

          const lastSeenStatus = user.status ? user.status.constructor.name.replace("UserStatus", "") : null;
          ...
          await push(ref(db, `exported_members/${userKey}`), {
            ...
            profilePhoto: profilePhotoURL, // store base64 link
